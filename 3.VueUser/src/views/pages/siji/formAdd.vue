					...
					context?.$http({
						...
						if(res.data.data.total>=crossOptNum){
							...
							context?.$http({
								...
								//加上用户的数据
								if(crossSession.value.xinyongfen>=0){
									crossSession.value.xinyongfen = parseFloat(crossSession.value.xinyongfen) + parseFloat(form.value.xinyongfen)
									context?.$http({url:`${context?.$toolUtil.storageGet('frontSessionTable')}/update`,method:'post',data:crossSession.value}).then(res=>{})
								}
